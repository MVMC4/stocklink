...
//!
//! `stocklink-identity migrate` applies pending migrations and exits.
...
use stocklink_identity::config::Config;
use stocklink_identity::routes;
use stocklink_identity::state::AppState;
...
async fn main() {
    ...
    if migrations_are_pending(&db).await {
        eprintln!("refusing to start: pending migrations. Run `stocklink-identity migrate` first.");
        std::process::exit(1);
    }
    ...
    tracing::info!(%host, port, "stocklink-identity listening");
