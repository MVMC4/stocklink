...
//!
//! `stocklink-notifications migrate` applies pending migrations and exits.
...
use stocklink_notifications::config::Config;
use stocklink_notifications::routes;
use stocklink_notifications::state::AppState;
...
async fn main() {
    ...
    if migrations_are_pending(&db).await {
        eprintln!(
            "refusing to start: pending migrations. Run `stocklink-notifications migrate` first."
        );
        std::process::exit(1);
    }
    ...
    tracing::info!(%host, port, "stocklink-notifications listening");
