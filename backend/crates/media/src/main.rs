...
//!
//! `stocklink-media migrate` applies pending migrations and exits.
...
use stocklink_media::config::Config;
use stocklink_media::routes;
use stocklink_media::state::AppState;
...
async fn main() {
    ...
    if migrations_are_pending(&db).await {
        eprintln!("refusing to start: pending migrations. Run `stocklink-media migrate` first.");
        std::process::exit(1);
    }
    ...
    tracing::info!(%host, port, "stocklink-media listening");
